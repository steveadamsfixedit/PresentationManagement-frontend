        ...
        <div class="py-6 px-6 lg:px-8 text-left">
          ...
          <form class="space-y-6">
            <div class="flex justify-center items-center w-full mt-2 mb-2">
              <label for="dropzone" class="flex flex-col justify-center items-center w-full h-64 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                ...
                <input id="dropzone" type="file" ref="file" class="hidden" v-on:change="handleUpload()" accept="*" />
              </label>
            </div>
          </form>
        </div>
...
const handleUpload = async() => {
  ...
  await presentationStore.uploadPowerpoint(props.id, formData);
  isOpen.value = false
};
</script>